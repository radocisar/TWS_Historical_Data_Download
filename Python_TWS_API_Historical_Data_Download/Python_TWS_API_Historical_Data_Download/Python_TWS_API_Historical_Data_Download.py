...
class New_App (EWrapper, EClient, Write_to_File):
    ...
    def historicalDataEnd(self, reqId:int, start:str, end:str):
        ...
        if dt.time(pd.to_datetime(end).hour, pd.to_datetime(end).minute, pd.to_datetime(end).second) == dt.time(15,59,59):
            Write_to_File.saving_Bars_to_File(self.Ticks_List, trading_date_list_item, Ticker_Symbol, Sec_Type_and_Currency)
        else:
            pass
...
def main():
    ...
    for stock in US_Stocks_Ticker_Dict.items():
        ...
        for trading_date in Trading_Dates_Reversed_List:
